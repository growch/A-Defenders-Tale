package view.capitol
{
	...
	public class DontDrinkView extends MovieClip implements IPageView
	{
		...
		private function init(e:ViewEvent) : void {
			...
			_pageInfo = DataModel.appData.getPageInfo("dontDrink");
			_bodyParts = _pageInfo.body;
			
			_mc.companionsDrink_mc.stop();
			_mc.companionsDrink_mc.companion_mc.stop();
			_mc.companionsDrink_mc.visible = false;
			
			_mc.end_mc.visible = false;
			
			var compInt:int = DataModel.defenderInfo.companion;
			_mc.companionsDont_mc.gotoAndStop(compInt + 1); // zero based
			
			//GRAPHICS
			DataModel.getInstance().setGraphicResolution(_mc.bg_mc);
			DataModel.getInstance().setGraphicResolution(_mc.end_mc);
			DataModel.getInstance().setGraphicResolution(_mc.companionsDont_mc.companion_mc);
			...
			for each (var part:StoryPart in _bodyParts) 
			{
				if (part.type == "text") {
					...
					if (part.id == "topText") {
						if (compInt == 0) {
							_mc.companionsDont_mc.y = _nextY - 20;
							_nextY += _mc.companionsDont_mc.height;
						} else if (compInt == 1) {
							_mc.companionsDont_mc.y = _nextY - 40;
							_nextY += _mc.companionsDont_mc.height - 40;
						} else {
							_mc.companionsDont_mc.y = _nextY + 10;
							_nextY += _mc.companionsDont_mc.height - 10;
						}
